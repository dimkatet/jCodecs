...
import type { JXLImageData, JXLDataType } from "./types";
import type { MainModule, DecodeResult } from "./wasm/jxl_dec_mt";
...
export async function decode(
  ...
  const descriptor = normalizeDescriptor(module as any, result.descriptor as any);
  const dataType = descriptor.numeric.dataType as JXLDataType;

  // Calculate element count based on data type
  const bytesPerElement = dataType === 'float32' ? 4 :
                          dataType === 'uint16' || dataType === 'float16' ? 2 : 1;
...
export async function decodeToImageData(
  input: Uint8Array | ArrayBuffer,
  options?: Omit<JXLDecodeOptions, "bitDepth">,
): Promise<ImageData> {
